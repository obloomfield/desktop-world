...
export function buildClouds() {
   ...
   for(let i = 0; i < numClouds; i++){
     ...
     var pos = new THREE.Vector3(THREE.MathUtils.randFloat(-300, 300), height, THREE.MathUtils.randFloat(-300, 300));
     var rotation = THREE.MathUtils.randFloat(0, 2 * Math.PI)
     cloudPos.push(pos);
     buildCloud(THREE.MathUtils.randFloat(10, 20), THREE.MathUtils.randFloat(200, 300), pos, rotation);
   }
}
...
export function makeRain(){
  ...
  for (let i = 0; i < clouds.length; i++) {
    ...
    for (let j = 0; j< 5; j++){
       let rainDropPos = new THREE.Vector3(cloudposition.x, cloudposition.y, cloudposition.z);
